...
const subContractBody = {
  ...
  amount_contract: Joi.number().min(0),
  amount_settlement: Joi.number().min(0).allow(null, ''),
  date_signed: Joi.date().allow(null, ''),
  remarks: Joi.string().allow(null, '').max(500),
  bond_perf_req: Joi.boolean(),
  bond_labor_req: Joi.boolean(),
  bond_perf_amt: Joi.number().min(0).allow(null, ''),
  bond_labor_amt: Joi.number().min(0).allow(null, ''),
