...
import { useInvoiceOut } from '@/hooks';
...
import { App, Button, Card, Col, Drawer, Row, Spin } from 'antd';
import dayjs from 'dayjs';
import React, { useEffect, useRef, useState } from 'react';
import { useReactToPrint } from 'react-to-print';
import { InvoiceOutPrintTemplate } from './InvoiceOutPrintTemplate';
...
const InvoiceOutDetail: React.FC<InvoiceOutDetailProps> = ({
  visible,
  currentRecord,
  onCancel,
}) => {
  ...
  const { data: detailResponse, isLoading, isFetching } = useInvoiceOut(recordId, detailEnabled);
  const typedResponse = detailResponse as { success?: boolean; data?: API.InvoiceOut } | undefined;
  ...
  const invoiceTitle = formatRecordDetailTitle(
    data?.mainContract?.contract_name ?? currentRecord?.mainContract?.contract_name,
    '销项发票详情',
  );

  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `开票申请_${data?.invoice_no || currentRecord?.invoice_no || ''}_${dayjs().format('YYYYMMDD')}`,
  });
  ...
  return (
    <Drawer
      ...
      destroyOnClose
      extra={
        <Button type="primary" icon={<PrinterOutlined />} size="small" onClick={handlePrint}>
          打印
        </Button>
      }
      styles={{ body: RECORD_DETAIL_DRAWER_BODY_STYLE }}
    >
      <Spin spinning={loading}>
        <Row gutter={16}>
          <Col span={16}>
            ...
            >
              <h3 style={sectionTitleStyle(MODULE_COLORS.invoiceOut.color, { isFirst: true })}>
              ...
              <Row gutter={24}>
                <Col span={8}>
                  <div
                    style={{
                      background: MODULE_COLORS.invoiceOut.bg,
                      borderRadius: '6px',
                      padding: '16px',
                      height: '100%',
                      display: 'flex',
                  ...
                  >
                    <div style={{ width: '100%' }}>
                      <div
                        style={{
                          fontSize: '14px',
                          color: MODULE_COLORS.invoiceOut.color,
                          marginBottom: '8px',
                          display: 'flex',
                          justifyContent: 'space-between',
                          alignItems: 'center',
                      ...
                      >
                        ...
                        {data.tax_rate !== null && data.tax_rate !== undefined && (
                          <span style={{ fontSize: '13px', color: MODULE_COLORS.invoiceOut.color }}>
                            税率: {data.tax_rate}%
                          </span>
                        )}
                      </div>
                    ...
                    <div
                      style={{
                        marginTop: '5px',
                        paddingTop: '5px',
                        borderTop: `1px dashed ${MODULE_COLORS.invoiceOut.color}`,
                        width: '100%',
              ...
              <Row gutter={24}>
                <DetailItem
                  label="合同名称"
                  value={data.mainContract?.contract_name || '-'}
                  span={12}
                />
                <DetailItem
                  label="合同金额"
                  value={
                    data.mainContract?.amount_contract != null
                      ? formatCurrency(data.mainContract.amount_contract)
                      : '-'
                  }
                  span={12}
                />
                <DetailItem label="付款方 (购买方)" value={data.buyer || '-'} span={12} />
                <DetailItem label="收款方 (销售方)" value={data.seller || '-'} span={12} />
              </Row>

              <h3 style={sectionTitleStyle(MODULE_COLORS.invoiceOut.color)}>
          ...
          <Col span={8}>
            <AttachmentPanel
              ...
              iconColor={MODULE_COLORS.invoiceOut.color}
      ...
      <div style={{ display: 'none' }}>
        <InvoiceOutPrintTemplate ref={printRef} invoiceData={data} />
      </div>
...
export default InvoiceOutDetail;
