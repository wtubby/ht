...
import { useRemoveInvoiceOut } from '@/hooks';
import { getInvoiceOuts } from '@/services/wtu/invoiceOut.api';
...
type InvoiceOutWithFiles = API.InvoiceOut & {
  has_files?: boolean;
};

interface InvoiceOutListProps {
  actionRef?: React.MutableRefObject<ActionType | undefined>;
  onViewDetail: (record: API.InvoiceOut) => void;
  onEdit: (record: API.InvoiceOut) => void;
  onCreate: () => void;
}

const InvoiceOutList: React.FC<InvoiceOutListProps> = ({
  ...
  const removeMutation = useRemoveInvoiceOut();

  const handleRemove = async (selectedRow: API.InvoiceOut) => {
  ...
  const columns: ProColumns<InvoiceOutWithFiles>[] = [
    {
      title: 'ID',
      dataIndex: 'id',
      search: false,
  ...
  return (
    <PageContainer pageHeaderRender={false}>
      <ProTable<InvoiceOutWithFiles, API.PageParams>
        headerTitle="销项发票管理"
        actionRef={actionRef}
        rowKey="id"
        search={false}
        ...
        request={async (params) => {
          const msg = await getInvoiceOuts({
            page: params.current,
        ...
        locale={{ emptyText: '还没有销项发票，请先有总包合同后再登记销项发票' }}
      />
    </PageContainer>
  );
};

export default InvoiceOutList;
